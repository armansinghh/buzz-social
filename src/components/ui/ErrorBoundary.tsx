...
// 1. Used in main.tsx to catch root/context initialization crashes
export default class ErrorBoundary extends Component<Props, State> {
  ...
  // Removals performed here to clear unused variable warnings safely

  public render() {
    if (this.state.hasError) {
      return <ErrorElementFallback />;
    }
...
// 2. Used in router.tsx as the errorElement for route crashes
export function ErrorElementFallback() {
  const handleReload = () => {
    window.location.href = "/"; // Hard reset routing tree back home cleanly
  };

  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-(--bg-primary) px-4 text-center select-none">
      <div className="max-w-sm space-y-4">
        {/* Styled friendly error graphic symbol */}
